...
| ID | 제목 | 설명 | 이슈 타입 | 상태 |
|---|---|---|---|---|
| **NB-25** | **Callout 블록 개선** | `...`으로 나오는 Callout 블록을 아이콘+내용이 보이도록 렌더러 수정 | Task | **Done** ✅ |
| **NB-26** | **에러 페이지 구현** | `404 Not Found` 및 `500 Error` 페이지 예쁘게 디자인 | Task | **Done** ✅ |
| **NB-27** | **파비콘 추가** | 브라우저 탭에 보일 로고 아이콘(`favicon.ico`) 적용 | Task | **Done** ✅ |
| **NB-16** | **다크 모드** | 사용자 시스템 설정에 따른 라이트/다크 모드 토글 기능 | Story | **Done** ✅ |
| **NB-17** | **검색 기능** | 글 제목이나 내용으로 검색하는 기능 추가 | Story | **Done** ✅ |
| **NB-22** | **Webhook 연동** | Notion 글 수정 시, 배포 없이도 사이트에 즉시 반영되도록(ISR) 설정 | Task | **Done** ✅ |
| **NB-28** | **비밀번호 보호 기능** | 특정 글을 비밀번호로 잠그기 (Server Action + Cookie) | Feature | **Done** ✅ |
...
| **NB-21** | **RSS 피드** | RSS 구독 지원 | Story | **Done** ✅ |
| **NB-23** | **보안 취약점 점검** | `npm audit` 경고 해결 | Task | **Done** ✅ |
